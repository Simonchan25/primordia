// GENESIS-OEE — an HONEST attempt at open-ended evolution (Node research sandbox).
//
// Two drivers the literature thinks matter for ongoing novelty:
//   (1) UNBOUNDED phenotype space — brains evolve their TOPOLOGY (NEAT-lite, recurrent):
//       mutations can add neurons and connections, so there is no fixed "best brain" to converge to.
//   (2) COEVOLUTION (Red Queen) — fitness depends on other evolving creatures (predator vs prey),
//       so the selective landscape never stops moving.
//
// We then MEASURE whether this stays open-ended or plateaus:
//   - mean & max brain complexity over time  (does it keep rising?)
//   - lineage turnover                        (does the dominant lineage keep being replaced?)
//   - trait diversity                         (do new strategies keep appearing?)
//   - predation rate                          (is the arms race active?)
// Honest expectation: nobody has achieved unbounded OEE. We report what actually happens.

export const P = {
  WORLD:1000, POP_CAP:700, POP_START:300, MIN_POP:25,
  FOOD_CAP:1400, FOOD_SPAWN:9, FOOD_ENERGY:34, START_ENERGY:60,
  ...
  CONN_COST:0.0028,      // metabolic cost per neural connection → complexity must pay for itself
  PLANT_FALLOFF:6,       // big bodies eat plants poorly → must hunt → forces a food chain (coevolution)
  // NEAT-lite mutation rates (per reproduction)
  W_PERTURB:0.85, W_SIG:0.4, W_RESET:0.08, ADD_CONN:0.07, ADD_NODE:0.035, TOGGLE:0.02,
  BODY_SIG:0.12, MAX_NODES:48, MAX_CONNS:150,
};
const NIN=9, NOUT=3, N0=NIN+NOUT;     // input + output node count (fixed)
...
// ---------- NEAT-lite genome ----------
// nodes: Int8Array-like array of types (0 input, 1 hidden, 2 output). indices: 0..NIN-1 inputs, NIN..N0-1 outputs, rest hidden.
// conns: array of {i,o,w,en}
...
function mutateGenome(g){
  ...
  // add connection (no duplicates; target not an input)
...
// recurrent one-step evaluation. act persists across ticks (memory). scratch sums reused.
const _sum=new Float32Array(P.MAX_NODES+8);
function evalBrain(g, act, inp, out){
  const nN=g.nodes.length;
  for(let k=0;k<NIN;k++) act[k]=inp[k];           // set inputs into current activations
  for(let k=0;k<nN;k++) _sum[k]=0;
  const cs=g.conns; for(let m=0;m<cs.length;m++){ const c=cs[m]; if(c.en) _sum[c.o]+=c.w*act[c.i]; }
  for(let k=NIN;k<nN;k++) act[k]=tanh(_sum[k]);    // hidden + outputs (uses prev act of sources = recurrence)
...
export function metrics(w){
  ...
  let dom=0,domLin=-1,distinct=0; for(const L in lc){ if(lc[L]>dom){dom=lc[L];domLin=+L;} if(lc[L]/pop>0.05)distinct++; }
...
// phenotype "classes" = coarse cells in (size, herbivore/carnivore, brain-complexity, speed) space
...
if(process.argv[1] && process.argv[1].endsWith('genesis_oee.mjs')){
  const w=makeWorld();
  console.log('OEE attempt · NEAT-lite recurrent brains + trophic predator-prey coevolution + complexity cost\n');
  const STEPS=240000, REP=10000; const novSeries=[]; const seen=new Set(); let t0=Date.now();
  for(let s=0;s<=STEPS;s++){ step(w);
    if(s>0 && s%REP===0){ const m=metrics(w);
      ...
      console.log(`t=${String(m.steps).padStart(6)} pop=${String(m.pop).padStart(3)} gen=${String(m.gen).padStart(4)} cx=${String(m.cxMean).padStart(5)} sizeStd=${m.sizeStd} carn=${m.carnFrac} occupied=${String(occ.size).padStart(3)} NEW=${nw} kills=${m.kills}`); } }
  const tail=novSeries.slice(Math.floor(novSeries.length*0.6));
  const lateNov=tail.reduce((a,b)=>a+b,0)/Math.max(1,tail.length);
  const ongoing = lateNov >= 1.0;
  console.log(`\n[OEE diagnostics — phenotype novelty]  (${((Date.now()-t0)/1000).toFixed(0)}s)`);
  console.log(`  distinct phenotype classes ever discovered: ${seen.size}`);
  console.log(`  NEW classes per 10k steps, late 40% of run: ${lateNov.toFixed(2)}`);
  console.log(ongoing
    ? '*** STILL GENERATING NOVELTY late in the run — open-ended-LEANING (the discovery rate never fell to zero).'
    : '... Novelty rate decayed toward zero — the space got exhausted (bounded). The honest, usual outcome.');
}
